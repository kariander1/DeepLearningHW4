...
class Episode(object):
    ...
    def calc_qvals(self, gamma: float) -> List[float]:
        ...
        # ====== YOUR CODE: ======
        q_prev = 0.0
        for i, experience in enumerate(reversed(self.experiences)):
            q_current = experience.reward + gamma * q_prev
            q_prev = q_current
            qvals.append(q_current)
        qvals.reverse()
        # ========================
        return qvals
...
class TrainBatch(object):
    ...
    def from_episodes(cls, episodes: Iterable[Episode], gamma=0.999):
        ...
        q_vals = torch.tensor(q_vals, dtype=torch.float)
    ...
    def episode_batch_generator(self) -> Iterator[Tuple[Episode]]:
        ...
        while True:
            ...
            while not episode_done:
                ...
                episode_done = experience.is_done
