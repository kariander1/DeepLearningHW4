...
class PolicyNet(nn.Module):
    def __init__(self, in_features: int, out_actions: int, **kw):
        ...
        modified_layers.append(out_actions)
        i_layer = 0
...
class PolicyAgent(object):
    ...
    def current_action_distribution(self) -> torch.Tensor:
        ...
        # Define no gradients saving since we are only inferring
        with torch.no_grad():
            current_scores = self.p_net(self.curr_state)
    ...
    def step(self) -> Experience:
        ...
        action = torch.multinomial(input=actions_proba, num_samples=1).item()
...
class VanillaPolicyGradientLoss(nn.Module):
    ...
    def _policy_loss(self, batch, action_scores, policy_weight):
        ...
        log_proba_selected = torch.gather(input=log_proba, dim=1, index=batch.actions.unsqueeze(dim=1)).squeeze()
        loss_p = -torch.mean(policy_weight * log_proba_selected)
        # ========================
        return loss_p
